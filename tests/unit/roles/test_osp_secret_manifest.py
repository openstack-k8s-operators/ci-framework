...
class TestOspSecretManifestCli:
    ...
    def test_set_only_reports_changed_keys(self, manifest_path, osp_secret_module):
        value = "existing-kek"
        encoded = base64.b64encode(value.encode()).decode()
        write_manifest(
            manifest_path,
        ...
        result = subprocess.run(
            [
                ...
                "set",
                str(manifest_path),
                json.dumps({"BarbicanSimpleCryptoKEK": value}),
            ],
            capture_output=True,
            text=True,
            check=False,
        ...
        result = subprocess.run(
            [
                ...
                "set",
                str(manifest_path),
                json.dumps({"BarbicanSimpleCryptoKEK": new_value}),
            ],
            capture_output=True,
            text=True,
            check=False,
