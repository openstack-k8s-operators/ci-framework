...
class ReportAccumulator(StreamResult):
    ...
    def get_results(self):
        ...
        for test_id, data in self.tests.items():
            ...
            if data['start_time']:
                entry['start_time'] = datetime.datetime.fromtimestamp(data['start_time']).strftime('%Y-%m-%d %H:%M:%S.%f')
                all_starts.append(data['start_time'])
            
            if data['end_time']:
                entry['end_time'] = datetime.datetime.fromtimestamp(data['end_time']).strftime('%Y-%m-%d %H:%M:%S.%f')
...
if __name__ == '__main__':
    ...
    parser.add_argument('output', nargs='?', default='tempest-viz.html', help='Output .html file (default: tempest-viz.html)')
