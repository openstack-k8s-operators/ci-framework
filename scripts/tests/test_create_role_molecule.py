...
class CreateRoleMoleculeTest(unittest.TestCase):
    ...
    def test_regenerate_projects_zuul_jobs_yaml(self):
        ...
        with open(self.generated_paths['ci_templates_dir'] / 'projects.yaml', 'w') as projects_file:
            yaml.dump(projects_jobs_info, projects_file)

        # Regenerate zuul.d/projects.yaml with the previous data + roles directories
        ...
        # We can't compare the list of jobs since the order matters. We need to convert it into a set
    ...
    def test_regenerate_molecule_zuul_jobs_yaml(self):
        ...
        with open(generated_paths['ci_config_dir'] / 'molecule.yaml', 'w') as projects_file:
        ...
        # Since the order matters for the asserts is a little bit complicated to test just with one assert
        for generated_dictionary in generated_molecule_zuul_jobs:
            ...
            # Find the corresponding expected dictionary that matches the generated dictionary
    ...
    def compareMoleculeJobDictionaries(self, dict1, dict2):
        ...
        self.assertEqual(dict1['job']['vars']['TEST_RUN'],
                         dict2['job']['vars']['TEST_RUN'])
