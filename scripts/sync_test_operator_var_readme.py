        ...
        if match:
            ...
            if var_name in defaults:
                ...
                for j in range(i, len(lines)):
                    ...
                    if j > i and re.match(r'^([a-zA-Z_][a-zA-Z0-9_]*)\s*:|^#\s*Section', lines[j]):
                        end_line = j - 1
                        break
...
def normalize_default_text(block, value, var_name, line_numbers):
        ...
        for var, block in existing.items():
            if var not in grouped_new[group]:
                ...
                if var in defaults:
                    ...
                    if not re.search(r"Default value:", block):
                        block = normalize_default_text(block, value, var, line_numbers)
