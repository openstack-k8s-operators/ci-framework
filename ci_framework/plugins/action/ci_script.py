...
class ActionModule(ActionBase):
    ...
    def __init__(self, **kwargs):
        ...
        self.__script_file_path = pathlib.Path().home().joinpath('ansible', 'tmp').joinpath(uuid.uuid4().hex).absolute()
        self.__script_file_path.parent.mkdir(parents=True, exist_ok=True)

    @staticmethod
    def __build_options(task_vars):
        if task_vars.get('cifmw_debug', False) or task_vars.get('cifmw_ci_script_debug', False):
            return 'set -x'
        return ''
    ...
    def run(self, tmp=None, task_vars=None):
        ...
        # Remove cmd if not passed, we are going to use _raw_params to pass the cmd we create here
        ...
        remote_script_path_str = output_dir.joinpath(f'ci_script_{fnum:03}_{t_name}.sh').as_posix()
        self._transfer_file(script_path_str, remote_script_path_str)
        self._fixup_perms2([remote_script_path_str], self._play_context.remote_user, execute=True)

        file_task = self._task.copy()
        file_task.args.update({
            '_raw_params': script_path_str,
            'chdir': output_dir.as_posix()
        })
