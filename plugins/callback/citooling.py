...
class CallbackModule(CallbackBase):
    ...
    '''
    This callback module output nested Ansible playbook execution.
    '''
...
#    CALLBACK_NEEDS_WHITELIST = False
    ...
    def __init__(self):
        ...
        self.nested_playbook = ""
        super(CallbackModule, self).__init__()

    def v2_runner_on_failed(self, result, ignore_errors=False):
        self._display.display(
            f"FAILED: [HOT: {result._host}, PLAYBOOK: {self.current_playbook}] =>{result.task_name}"
        )
    ...
    def v2_runner_on_ok(self, result):
        host_label = self.host_label(result)
        current_task_name = result.task_name.replace('Run', '')
        ...
        if 'cmd' in result._result:
            ...
            if result._result.get("_ansible_no_log"):
                self._display.display(
                    f"OK: [HOST: {host_label}, "
                    f"\n\tPLAYBOOK: {self.current_playbook}] "
                    f"\n\t{current_task_name} CENSORED"
                )
                return

            if "ansible-playbook" in cmd:
                self.nested_playbook = cmd.split(' ')[1]
                self._display.display(
                    f"=>  Nested playbook execution from "
                    f"[HOST: {host_label}, PLAYBOOK: {self.current_playbook}]"
                    f"\n└── {current_task_name}, \n\tCOMMAND:\n{cmd}\n"
                )

            if self.nested_playbook:
                self._display.display(
                    f"\tOK: [NESTED PLAY, HOST: {host_label}, PLAYBOOK: {self.current_playbook}] "
                    f"\t\n\t=>{current_task_name}, \n\tCOMMAND:\n{cmd}\n\t=> OUTPUT: {stdout}\nEND - {end} - delta: {delta}"
                )
            else:
                self._display.display(
                    f"OK: [HOST: {host_label}, PLAYBOOK: {self.current_playbook}] "
                    f"\n\t=> {current_task_name}, \n\tCOMMAND:\n{cmd}\n\t=> OUTPUT: {stdout}\nEND - {end} - delta: {delta}"
                )
    ...
    def v2_runner_on_skipped(self, result):
        if self.nested_playbook:
            self._display.display(
                f"SKIPPING: [HOST: {result._host}, "
                f"\n\t=> PLAYBOOK: {self.nested_playbook}] "
                f"\n\t=> {result.task_name}",
                color=C.COLOR_SKIP
            )
...
#            self._display.display("daniel %s" % self.__dict__)

        if self.nested_playbook:
            self._display.display(
                f"\nOK: [NESTED PLAY, PLAYBOOK: {self.nested_playbook}] "
                f"\n\t=>{task}"
            )
        ...
        if self.current_playbook:
            self._display.display(
                f"\nOK: [NESTED PLAY, PLAYBOOK: {self.current_playbook}] "
                f"\n\t=>{task}"
            )
        else:
            self._display.display(
                f"\nOK: [PLAYBOOK: {self.current_playbook}]"
                f"\n\t=> {task}"
            )
    ...
    def _task_start(self, task, prefix=None):
        if self.nested_playbook:
            self._display.display(
                f"\nOK: [NESTED PLAY: {self.nested_playbook}] "
                f"\n\t=>{task}"
            )
        elif not self.nested_playbook and self.current_playbook:
            self._display.display(
                f"\nOK: [TASKS START IN PLAYBOOK: {self.current_playbook}] "
                f"\n\t=>{task}"
            )
        elif self.current_playbook:
            test = self.__dict__
            self._display.display(
                f"\nOK: [TASKS START IN PLAYBOOK: UNKNOWN] "
                f"\n\t=>{test}\n\n"
                f"\n\t=>{task}"
            )
...
#        self._display.display(" [started %s on %s]" % (task, host), color=C.COLOR_OK)
    ...
    def v2_playbook_on_play_start(self, play):
        ...
        self.current_playbook = name.split(' ')[1]
...
#            self._display.banner("DRY RUN")
    ...
    def v2_playbook_on_start(self, playbook):
        if self.nested_playbook:
            self.current_playbook = self.nested_playbook
        elif not self.current_playbook:
            self.current_playbook = basename(playbook._file_name)
