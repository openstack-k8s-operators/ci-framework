...
* `cifmw_reproducer_controller_user`: (String) User on controller-0. Defaults to `USER` from controller-0's `ansible_env` if available, otherwise defaults to `zuul`.
* `cifmw_reproducer_controller_user_dir`: (String) Controller-0 user's home dir. Defaults to `HOME` from controller-0's `ansible_env` if available, otherwise defaults to `/home/{{ cifmw_reproducer_controller_user }}`
