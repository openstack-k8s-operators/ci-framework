...
class RedfishHandler(BaseHTTPRequestHandler):
                ...
                "Oem": {
                    "Dell": {
                        "DellSystem": (
                            ...
                            if STATE.post_state is not None
                            else {}
                        ),
                    }
                },
    ...
    def _get_boot_options_collection(self):
        visible = {
            k: v for k, v in BOOT_OPTIONS.items() if k not in STATE.hidden_boot_options
        }
        self._send_json(
            {
                "@odata.id": "/redfish/v1/Systems/System.Embedded.1/BootOptions",
                "Members": [
                    {"@odata.id": opt["@odata.id"]} for opt in visible.values()
                ],
                "Members@odata.count": len(visible),
            }
        )

    def _get_boot_option(self, boot_id):
        if boot_id in BOOT_OPTIONS and boot_id not in STATE.hidden_boot_options:
        ...
        self._send_json(
            {
                ...
                "hidden_boot_options": STATE.hidden_boot_options,
            }
        )
