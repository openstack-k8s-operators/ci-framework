    ...
    *search_paths : str
    ...
    for path in search_paths:
        if os.path.isfile(path):
            ...
            sources.extend(os.path.join(path, file)
                           for file in os.listdir(path)
                           if os.path.isfile(os.path.join(path, file)))

        else:  # try glob expansion
            sources.extend(candidate for candidate in glob(path)
...
def load_csv(path: str, source: str | None = None) -> pd.DataFrame:
    ...
    # TODO(sdatko): This is inaccurate, make tool return the physmem
    df['mem.used'] = (df['mem.util.bufmem'] + df['mem.util.cached']
                      + df['mem.util.slab'] + df['mem.util.mapped'])
    df['mem.total'] = df['mem.freemem'] + df['mem.used']
    # y=(100 * df['mem.used'] / df['mem.total']),
    # y2=(100 * df['mem.util.mapped'] / df['mem.total']),
...
def set_xaxis(axs: Iterable[plt.Axes]) -> None:
    ...
    for arg in ({'minor': False}, {'minor': True}):
        ax.set_xticks(ax.get_xticks(**arg), ax.get_xticklabels(**arg), **arg,
                      size=8, rotation=45, ha='right', rotation_mode='anchor')
...
def set_legend(fig: plt.Figure, axs: Iterable[plt.Axes]) -> None:
    ...
    axs : Iterable[plt.Axes]
        A collection of subplot objects in which the data were plotted.
    '''
    # TODO: merge unique entries?
    # handles_labels = [ax.get_legend_handles_labels() for ax in axs]
    # handles, labels = [sum(hl, []) for hl in zip(*handles_labels)]
...
if __name__ == '__main__':
    try:
        ...
        for hostname in sorted(df['source'].unique()):
            ...
            plot(df.query(f'source == "{hostname}"'),
                 output=path,
                 title=hostname)
