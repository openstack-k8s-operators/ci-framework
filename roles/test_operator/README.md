...
* `cifmw_test_operator_generate_stackviz`: (Boolean) Enable automatic generation of Stackviz HTML reports from Tempest subunit test results. When enabled, generates interactive visualizations of test results that can be viewed in a browser. When `cifmw_test_operator_tempest_rerun_failed_tests` is enabled, stackviz will generate both `tempest-viz.html` (original test run) and `tempest_retry_viz.html` (retry test run) reports for easy comparison. Default value: `true`
...
* `cifmw_test_operator_stackviz_auto_install_deps`: (Boolean) Automatically install required Python dependencies (python-subunit, testtools) for Stackviz generation if they are missing. When disabled, the role will fail with installation instructions if dependencies are not found. Default value: `true`
