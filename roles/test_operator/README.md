...
* `cifmw_test_operator_controller_priv_key_file_path`: (String) Specifies the path to the CIFMW private key file. Note: The test-operator searches for this key file locally. Please ensure the file is available on the same machine that is hosting the test-operator. Default value: `~/.ssh/id_cifw`
