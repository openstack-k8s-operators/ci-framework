...
`qcow_prefix`, and `images_file`) set. Assuming these conditions are met,
and an image can be discovered for each provided `base_url`,
...
Every time the `discover_latest_image` role is called, the
`cifmw_discovered_images_dict` is reset, and the single-image
`cifmw_discovered_[image_name,image_url,hash,hash_algorithm]` facts are set to
`null`, erasing any discovered image data that may exist from previous calls.
Either call the role once with all of the image requests you intend to make, or
be careful to manage the complete collection of discovered images yourself.
