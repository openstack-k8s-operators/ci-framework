...
* `cifmw_shiftstack_k8s_cp_retries`: (*integer*) Retries when copying the command script into the pod with `k8s_cp`. Defaults to `12`.
* `cifmw_shiftstack_k8s_cp_delay`: (*integer*) Seconds between `k8s_cp` retry attempts. Defaults to `10`.
* `cifmw_shiftstack_diagnostics_dir`: (*string*) Directory for cluster diagnostic dumps on command failure. Defaults to `"{{ cifmw_shiftstack_basedir }}/diagnostics"`.
